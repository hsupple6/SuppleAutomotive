...
/** Matches top-level section headings: ## 1. Title or ## § 1. Title (§ appears in PDFs as UTF-8). */
var AGREEMENT_SECTION_HEADING = /\n(?=##\s+(?:§\s*)?\d+\.\s)/;

function parseAgreementMd(source) {
  var idx = source.search(AGREEMENT_SECTION_HEADING);
  if (idx === -1) {
    return { cover: source.trim(), sections: [] };
  }
  var cover = source.slice(0, idx).trim();
  var rest = source.slice(idx + 1);
  var parts = rest.split(AGREEMENT_SECTION_HEADING);
