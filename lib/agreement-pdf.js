...
var AGREEMENT_RELEASE_FIELD_SPEC = [
  ...
  {
    key: 'authorized_repair_amount',
    label: 'Authorized repair amount (without extra approval)',
    required: true
  },
  ...
  { key: 'labor_warranty_days', label: 'Labor warranty (days)', required: true },
  {
    key: 'addendum',
...
function releasePlaceholderMap(fields) {
  ...
  return {
    ...
    __AUTHORIZED_REPAIR_AMOUNT__: String(f.authorized_repair_amount || '').trim(),
    ...
    __WARRANTY_DAYS__: String(f.labor_warranty_days || '').trim()
  };
}
  ...
  return new Promise(function (resolve, reject) {
    ...
    function renderBodyWithSigMarkers(body) {
      if (!body || body.indexOf('[[CUSTOMER_SIG]]') === -1) {
        doc.fontSize(10).font('Helvetica').fillColor('#111').text(body || '', {
          width: contentW,
          lineGap: 3,
          align: 'left'
        });
      ...
      for (var pi = 0; pi < parts.length; pi++) {
        var chunk = parts[pi];
        if (chunk) {
          doc.fontSize(10).font('Helvetica').fillColor('#111').text(chunk, {
            width: contentW,
            lineGap: 3,
            align: 'left'
          });
