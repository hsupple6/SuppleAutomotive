...
function buildInvoicePdfBuffer(opts) {
  return new Promise(function (resolve, reject) {
    ...
    y += 10;
    doc.font('Helvetica-Bold').text('Vehicle information', margin, y);
    y += 14;
    doc.font('Helvetica').fontSize(10).fillColor('#333333');
    if (vehicle) {
      var vehDesc = [vehicle.year, vehicle.make, vehicle.model].filter(Boolean).join(' ');
      if (vehDesc) {
        doc.text(vehDesc, margin, y, { width: contentW });
        y += 14;
      }
      var vehMeta = [];
      if (vehicle.mileage != null && vehicle.mileage !== '') vehMeta.push('Odometer: ' + String(vehicle.mileage));
      if (vehicle.license_plate) vehMeta.push('License: ' + String(vehicle.license_plate));
      if (vehicle.trim) vehMeta.push('Engine/Trim: ' + String(vehicle.trim));
      if (vehicle.vin) vehMeta.push('VIN: ' + String(vehicle.vin));
      if (vehMeta.length) {
        doc.text(vehMeta.join(', '), margin, y, { width: contentW });
        y += 14;
      }
    } else {
      doc.text('—', margin, y);
    ...
    doc.font('Helvetica-Bold').text('Service', margin, y);
    y += 14;
    doc.font('Helvetica');
    doc.text(service.service_name || 'Service', margin, y, { width: contentW });
    y += 22;
