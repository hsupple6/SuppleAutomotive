...
function normalizePart(raw) {
  return {
    ...
    cost: String((raw && raw.cost) != null ? raw.cost : '').trim()
  };
}
...
function validateEstimateReleaseFieldsComplete(fields) {
  ...
  f.services.forEach(function (s, i) {
    ...
    (s.parts || []).forEach(function (p, j) {
      ...
      if (!String(p.cost || '').trim()) missing.push('services[' + i + '].parts[' + j + '].cost');
    });
  });
  return missing;
}
...
function buildEstimatePdfBuffer(customer, vehicle, options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    doc.rect(margin, doc.y, contentW, 88).strokeColor('#bbb').lineWidth(0.75).stroke();
    var authBodyTop = doc.y + 8;
    doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#111').text('Original Estimate: ' + money(grandTotal), margin + 8, authBodyTop);
    doc.font('Helvetica').fontSize(8).fillColor('#333').text('Authorized By:', margin + 8, authBodyTop + 16);
    if (hasCustomerSig && customerSignature.mode === 'typed') {
      doc.font('Helvetica-Oblique').fontSize(14).text(String(customerSignature.payload || '').slice(0, 90), margin + 72, authBodyTop + 12);
    } else if (hasCustomerSig && customerSignature.mode === 'drawn') {
      doc.text('[Signature on file]', margin + 72, authBodyTop + 14);
    } else {
      doc.text('_____________________________________', margin + 72, authBodyTop + 16);
    }

    var authMidX = margin + 250;
    doc.font('Helvetica').fontSize(7.5).fillColor('#333');
    doc.text('Date: ' + signedDateDisplay, authMidX, authBodyTop);
    doc.text('Time: ' + (authTimeStr || '_______________'), authMidX, authBodyTop + 12);
    doc.text('Phone #: ' + (authPhoneStr || '_______________'), authMidX, authBodyTop + 24);
    doc.text('Email: ' + (authEmailStr || '_______________'), authMidX, authBodyTop + 36);

    var checkX = margin + 430;
    var checkY = authBodyTop;
    ...
    doc.y = authBodyTop + 52;
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#111').text('Additional authorization (if needed)', margin + 8, doc.y);
