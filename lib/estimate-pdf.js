...
/** Percent for parts tax; strips % and non-numeric except . and - */
  ...
  out.parts = parts
    .map(function (p) {
      return {
        ...
        labor_hours: String((p && p.labor_hours) || '').trim()
      };
    })
    .filter(function (p) {
...
function validateEstimateReleaseFieldsComplete(fields) {
  ...
  f.parts.forEach(function (p, i) {
    ...
    if (!String(p.labor_hours || '').trim()) missing.push('parts[' + i + '].labor_hours');
  });
  return missing;
}
...
function buildEstimatePdfBuffer(customer, vehicle, options) {
  ...
  var parts = fields.parts.map(function (p) {
    ...
    var partHours = normalizeHours(p.labor_hours);
    var partLaborTotal = partHours * laborRate;
    return {
      name: p.name || 'Part',
      line_label: 'Remove and Replace',
      cost: partCost,
      labor_hours: partHours,
      labor_total: partLaborTotal,
      line_total: partCost + partLaborTotal
    };
  });
  var serviceLaborTotal = services.reduce(function (sum, s) {
    return sum + s.labor_total;
  }, 0);
  var partsLaborHours = parts.reduce(function (sum, p) {
    return sum + p.labor_hours;
  }, 0);
  var partsLaborTotal = partsLaborHours * laborRate;
  var laborTotal = serviceLaborTotal + partsLaborTotal;
  var partsTotal = parts.reduce(function (sum, p) {
    return sum + p.cost;
  }, 0);
  var partsTaxPercent = normalizePartsTaxPercent(fields.parts_tax_percent, 7.25);
  var partsTaxAmount = Math.round(partsTotal * (partsTaxPercent / 100) * 100) / 100;
  var grandTotal = laborTotal + partsTotal + partsTaxAmount;
  ...
  return new Promise(function (resolve, reject) {
    ...
    doc.text('Parts tax: ' + formatPercentLabel(partsTaxPercent) + '%', rightX - 170, margin + 32, { width: 170, align: 'right' });
    ...
    parts.forEach(function (p) {
      ...
      doc.text(String(p.labor_hours.toFixed(2)), margin + 300, doc.y, { width: 50, align: 'right' });
      doc.text(money(p.line_total), rightX - 78, doc.y, { width: 72, align: 'right' });
      doc.font('Helvetica').fontSize(8).fillColor('#666');
      doc.text('Part cost ' + money(p.cost), margin + 12, doc.y + 11, { width: 220 });
    ...
    doc.y += 16;
    doc.font('Helvetica').fontSize(10).fillColor('#333');
    doc.text('Parts tax (' + formatPercentLabel(partsTaxPercent) + '%)', rightX - 170, doc.y, { width: 90 });
    doc.text(money(partsTaxAmount), rightX - 78, doc.y, { width: 72, align: 'right' });
    doc.y += 18;
    ...
    doc.font('Helvetica').fontSize(9).fillColor('#333');
    doc.text(
      'Customer authorizes Supple Automotive to perform the services listed in this estimate at the stated price.',
      { width: contentW, lineGap: 3, align: 'left' }
    );
    doc.moveDown(0.5);
    doc.text(
      'Customer acknowledges that this estimate is based on current inspection and may not include all necessary repairs. Any additional work or cost will require customer approval prior to proceeding.',
      { width: contentW, lineGap: 3, align: 'left' }
    );
    doc.moveDown(0.5);
    doc.text(
      'Customer agrees to the labor, services, parts, and charges listed in this estimate as shown above, except as modified with customer approval prior to proceeding.',
      { width: contentW, lineGap: 3, align: 'left' }
    );
    doc.moveDown(1);
    ensureSpace(72);
    ...
    var hasRenderedMechanicSig = false;
    ...
    if (fs.existsSync(dancingWoff)) {
      try {
        ...
        hasRenderedMechanicSig = true;
      } catch (e) {
        // Fall back to Helvetica-Oblique signature.
      }
    }
    if (!hasRenderedMechanicSig) {
