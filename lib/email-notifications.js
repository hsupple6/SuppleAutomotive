...
function sendDocumentsAssignedAlert(payload) {
  var body = '' +
    ...
    '<p style="margin:0 0 12px;font-size:15px;line-height:1.7;color:#344054;">Please open your user portal to review your account and complete any next steps.</p>';
  return sendEmail(
    payload.email,
    ...
    return mirrorCustomerSms(payload, [
      'Supple Automotive: New documents on your account.',
...
function sendSignedDocumentAlert(payload) {
  ...
  var body = '' +
    ...
    '<p style="margin:0 0 12px;font-size:15px;line-height:1.7;color:#344054;">You can open it directly below or view it anytime in your user portal documents.</p>';
  return sendEmail(
    payload.email,
    ...
    baseLayout({
      ...
      actionUrl: payload.documentUrl || portalUrlForEmail(payload, getBranding().paymentPortalUrl)
    })
  ).then(function (r) {
    var smsLines = [
      'Supple Automotive: Your signed ' + String(payload.documentType || 'document') + ' is ready.',
      'PDF: ' + String(payload.documentUrl || '').slice(0, 120)
    ];
