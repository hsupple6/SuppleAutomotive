...
/** Email a PDF copy to any address (public document link flow). */
function sendDocumentPdfCopy(payload) {
  var email = String(payload.email || '').trim();
  if (!email) return Promise.resolve({ skipped: true, reason: 'missing-email' });
  var docType = String(payload.documentType || 'document').toLowerCase();
  var docLabel = docType === 'estimate' ? 'estimate' : docType === 'invoice' ? 'invoice' : 'document';
  var hasAttachment = payload.pdfBuffer && Buffer.isBuffer(payload.pdfBuffer) && payload.pdfBuffer.length > 0;
  var body =
    '<p style="margin:0 0 12px;font-size:15px;line-height:1.7;color:#344054;">Attached is your ' +
    escapeHtml(docLabel) +
    ' from Supple Automotive.</p>';
  if (payload.documentUrl) {
    body +=
      '<p style="margin:0;font-size:14px;line-height:1.6;color:#475467;"><a href="' +
      escapeHtml(payload.documentUrl) +
      '" style="color:#0f1720;">View online</a></p>';
  }
  var attachment = hasAttachment
    ? pdfAttachmentFromBuffer(payload.pdfBuffer, payload.pdfFilename || docLabel + '.pdf')
    : null;
  return sendEmail(
    email,
    payload.subject || 'Your ' + docLabel + ' from Supple Automotive',
    baseLayout({
      heading: payload.heading || 'Your document',
      intro: payload.intro || 'Thank you for choosing Supple Automotive.',
      bodyHtml: body
    }),
    attachment ? { attachments: [attachment] } : undefined
  );
...
module.exports = {
  ...
  sendDocumentPdfCopy: sendDocumentPdfCopy
};
